...
def segment_asot(cost_matrix, mask_X=None, mask_Y=None, eps=0.07, alpha=0.3, radius=0.04, ub_frames=False,
    ...
    # We use N-1 here to replicate the mask given by the dataloader, which is of size N(before adding virtual frame) AKA N-1
    if mask_X is None:
        mask_X = torch.full((B, N-1), 1, dtype=bool, device=dev)
    if mask_Y is None:
        mask_Y = torch.full((B, N-1), 1, dtype=bool, device=dev)
